...
WINDOW_SIZE    = 500     # trigger monitoring cycle every N samples
...
# Train RandomForest on reference window (raw features, before scaling)
...
ml_model.fit(reference_X, reference_y)
print(f"[Server] RandomForest trained on {len(reference_X)} reference samples.")
...
def predict(request: PredictRequest, background_tasks: BackgroundTasks):
    ...
    4. When buffer reaches WINDOW_SIZE → triggers background monitoring cycle
    """
    ...
    with buffer_lock:
        ...
        if current_buffer_size >= WINDOW_SIZE:
            ...
            batch_labels_arr = (
                np.array(buffer_labels, dtype=np.int32) if buffer_labels else None
            )
    ...
    return PredictResponse(
        ...
        batch_buffered=min(current_buffer_size, WINDOW_SIZE),
    )


@app.get("/monitoring/latest")
def monitoring_latest():
    """Latest monitoring result — Streamlit polls this for live updates."""
    with history_lock:
...
def get_config():
    ...
    return {
        "active_classifier": ml_model.active_model_name,
        "available_classifiers": list(ml_model.models.keys()),
        "window_size": WINDOW_SIZE,
        "p_value_threshold": calibration.DRIFT_P_VALUE_THRESHOLD,
    }
...
def update_config(config: ConfigSchema):
    ...
    global WINDOW_SIZE
    if config.active_classifier is not None:
        try:
            ml_model.set_active_model(config.active_classifier)
            print(f"[Server] Active classifier switched to: {config.active_classifier}")
        except ValueError as e:
            raise HTTPException(400, str(e))
            
    if config.window_size is not None:
        if config.window_size < 10 or config.window_size > 5000:
            raise HTTPException(400, "Window size must be between 10 and 5000.")
        WINDOW_SIZE = config.window_size
        print(f"[Server] Dynamic window size set to: {WINDOW_SIZE}")
        
    if config.p_value_threshold is not None:
        if config.p_value_threshold <= 0.0 or config.p_value_threshold > 1.0:
            raise HTTPException(400, "p-value threshold must be between 0 and 1.")
        calibration.DRIFT_P_VALUE_THRESHOLD = config.p_value_threshold
        print(f"[Server] Drift statistical p-value threshold set to: {calibration.DRIFT_P_VALUE_THRESHOLD}")
...
def admin_simulate(n_stable: int = 1000, n_drift: int = 1500):
    ...
    n_stable: samples from current_stable window (no drift)
    n_drift:  samples from current_drift window (drift injected)
    """
    ...
    # Vectorised: scale all features at once, predict all at once
    ...
    # Split into WINDOW_SIZE batches and trigger monitoring
    for start in range(0, injected, WINDOW_SIZE):
        end = min(start + WINDOW_SIZE, injected)
