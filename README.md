...
```
┌─────────────┐     POST /predict      ┌──────────────────────────────────┐
│ IoT Client  │ ─────────────────────► │  FastAPI (port 8000)             │
│ example_    │                        │  • Dummy ML model → prediction   │
│ client.py   │                        │  • Sample buffer (500 samples)   │
└─────────────┘                        │  • Background monitoring trigger │
                                       └──────────────┬───────────────────┘
                                                      │ every 500 samples
                                                      ▼
                                       ┌──────────────────────────────────┐
                                       │  RadarOrchestrator (7 layers)    │
                                       │  SMOTE → AE drift → RNN unc →    │
                                       │  MHS → SHAP → Retrain → Alerts   │
                                       └──────────────┬───────────────────┘
                                                      │ MonitoringResult
                                                      ▼
┌─────────────┐     polls every 3s     ┌──────────────────────────────────┐
│ Next.js     │ ◄───────────────────── │  In-memory monitoring history    │
